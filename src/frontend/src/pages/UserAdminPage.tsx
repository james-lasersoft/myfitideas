...
export default function UserAdminPage() {
  ...
  return (
    <main className="admin-page security-admin-page">
      <AdminPageHeader
        eyebrow={t("Administration / Users")}
        title={t("User Management")}
        description={t("Invite users, assign roles, control account status, and revoke active sessions.")}
        ...
        <section className="security-panel user-role-editor" aria-labelledby="role-editor-title">
          <div className="security-toolbar role-editor-heading">
            <div>
              ...
              <p><strong>{selectedUser.user.firstName} {selectedUser.user.lastName ?? ""}</strong><span>{selectedUser.user.email}</span></p>
            </div>
          ...
          <div className="user-role-grid">
              ...
              <div className="role-checkbox-list">
                {activeRoles.map((role) => (
                  <label key={role.id} className="permission-option role-assignment-option">
                    <input type="checkbox" checked={selectedRoleIds.includes(role.id)} onChange={(event) => setSelectedRoleIds((current) => event.target.checked ? [...current, role.id] : current.filter((roleId) => roleId !== role.id))} />
                    <span><strong>{role.name}</strong><small>{role.description ?? t("No description available.")}</small></span>
                  </label>
                ))}
              </div>
            </section>
            ...
            <section className="effective-permissions-panel">
              ...
              {effectivePermissions.length === 0 ? <p className="empty-state">{t("No roles selected.")}</p> : <div className="permission-chip-list">{effectivePermissions.map((permission) => <code key={permission}>{permission}</code>)}</div>}
            </section>
          </div>
          ...
          <div className="profile-actions">
            <Button variant="outline" onClick={closeRoleEditor}>{t("Cancel")}</Button>
            <Button onClick={() => void saveRoles()} disabled={savingRoles}>{savingRoles ? t("Saving...") : t("Save Roles")}</Button>
          </div>
        </section>
      )}
      ...
      <section className="security-panel">
        <div className="security-toolbar user-list-toolbar">
          <input value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void applySearch(); } }} placeholder={t("Search users")} />
        ...
        {loading ? (
          <AdminLoadingState label={t("Loading users...")} />
        ) : (
          <div className="security-table-wrap">
            <table className="security-table">
              <thead><tr><th>{t("User")}</th><th>{t("Status")}</th><th>{t("Roles")}</th><th>{t("Last login")}</th><th>{t("Actions")}</th></tr></thead>
              <tbody>
                {users.map((item) => {
                  ...
                  return (
                    <tr key={item.membershipId} aria-busy={updating}>
                      ...
                      <td>{canUpdateUsers ? <select value={item.user.status} onChange={(event) => void changeStatus(item, event.target.value)} disabled={updating}><option value="ACTIVE">{t("Active")}</option><option value="INACTIVE">{t("Inactive")}</option><option value="SUSPENDED">{t("Suspended")}</option></select> : <span className={`status-badge ${statusClass(item.user.status)}`}>{statusLabel(item.user.status, t)}</span>}</td>
                      <td><div className="assigned-role-list">{item.roles.length === 0 ? <span>{t("No roles assigned")}</span> : item.roles.map((role) => <span key={role.id} className="role-badge">{role.name}</span>)}</div></td>
                      <td>{item.user.lastLoginAt ? new Date(item.user.lastLoginAt).toLocaleString() : t("Never")}</td>
                      <td><div className="table-action-group">{canAssignRoles && <Button size="sm" onClick={() => openRoleEditor(item)} disabled={updating}>{t("Manage Roles")}</Button>}{canRevokeSessions && <Button variant="outline" size="sm" onClick={() => void revokeSessions(item)} disabled={updating}>{t("Revoke Sessions")}</Button>}{!canAssignRoles && !canRevokeSessions && <span className="view-only-label">{t("View only")}</span>}{updating && <span className="inline-progress"><span className="admin-spinner small" aria-hidden="true" />{t("Saving...")}</span>}</div></td>
                    </tr>
                  );
                })}
              </tbody>
