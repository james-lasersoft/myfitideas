...
export default function ProgressChartsPage() {
  ...
  const [chartData, setChartData] =
    useState<ProgressChartData | null>(null);

  const [range, setRange] =
    useState<ChartRange>("30");
  ...
  useEffect(() => {
    async function loadChartData() {
      try {
        ...
        console.error(
          "Unable to load progress chart data:",
          loadError
        );

        setError(
          "Unable to load progress charts. Please try again."
        );
      } finally {
        setLoading(false);
      }
  ...
  const filteredMeasurements = useMemo(() => {
    ...
    return filterMeasurementsByRange(
  ...
  const filteredHydration = useMemo(() => {
    ...
    return filterHydrationByRange(
      chartData.hydrationEntries,
      range
    );
  }, [chartData, range]);
  ...
  const hydrationTotals = useMemo(() => {
    ...
    return groupHydrationByDate(
      filteredHydration,
      chartData.profile.preferredHydrationUnit,
      range
    );
  }, [chartData, filteredHydration]);

  const weightUnit =
    chartData?.profile.preferredWeightUnit ?? "lb";

  const hydrationUnit =
    chartData?.profile.preferredHydrationUnit ?? "oz";

  const measurementLabels = filteredMeasurements.map(
    (measurement) =>
  ...
  const weightValues = filteredMeasurements.map(
    (measurement) => measurement.weight
  );

  const bodyFatValues = filteredMeasurements.map(
    (measurement) => measurement.bodyFat
  );

  const waistValues = filteredMeasurements.map(
    (measurement) => measurement.waist
  );

  const chestValues = filteredMeasurements.map(
    (measurement) => measurement.chest
  );

  const hipsValues = filteredMeasurements.map(
    (measurement) => measurement.hips
  );

  const hasWeightData =
    hasMeasurementValues(weightValues);

  const hasBodyFatData =
    hasMeasurementValues(bodyFatValues);
    ...
    <main className="progress-charts-page">
      <section className="progress-charts-header">
        <div>
          ...
          <p>
            Review changes in your measurements and
            hydration over time.
          </p>
        </div>
        ...
        >
          {[
            ...
            <button
              ...
              onClick={() =>
                setRange(option.value as ChartRange)
              }
            >
              {option.label}
            </button>
        ...
        <section className="charts-grid">
          <article className="chart-card chart-card-wide">
            ...
            {hasWeightData ? (
              ...
              <div className="chart-empty-state">
                No weight measurements are available for
                this period.
              </div>
            )}
          </article>
          ...
          <article className="chart-card">
            ...
            {hasBodyFatData ? (
              ...
              <div className="chart-empty-state">
                No body-fat measurements are available
                for this period.
              </div>
            )}
          </article>
          ...
          <article className="chart-card">
            <div className="chart-card-header">
              <div>
                ...
                <p>
                  Total hydration logged each day.
                </p>
              </div>
            ...
            {hydrationTotals.length > 0 ? (
              ...
              <div className="chart-empty-state">
                No hydration entries are available for
                this period.
              </div>
            )}
          </article>
          ...
          <article className="chart-card chart-card-wide">
            ...
            {hasBodyMeasurementData ? (
              ...
              <div className="chart-empty-state">
                No body measurements are available for
                this period.
              </div>
            )}
          </article>
