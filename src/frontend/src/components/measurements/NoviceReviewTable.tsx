...
import { CORE_MEASUREMENT_FIELDS, FIELD_LABELS, PAIRED_MEASUREMENT_FIELDS, type SessionField } from "./measurementSessionModel";
...
export default function NoviceReviewTable({ values, lengthUnit, onEdit }: NoviceReviewTableProps) {
  ...
  return <div className="measurement-table-wrap">
    <table className="measurement-table novice-review-table">
      ...
      <tbody>
        {CORE_MEASUREMENT_FIELDS.map(({ field, stepIndex }) => {
        ...
        {PAIRED_MEASUREMENT_FIELDS.map(({ title, left, right, stepIndex }) => {
